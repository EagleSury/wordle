...
#
    ...
    def test_format_guess_01(self):
        ...
        observed = wordle_engine.format_guess('fghij', 'fghij')
        expected = f"{wc.GREEN}f{wc.GREEN}g{wc.GREEN}h{wc.GREEN}i{wc.GREEN}j{wc.ENDC}"
        self.assertEqual(expected, observed)

    def test_format_guess_01(self):
        # some matching letters and some in wrong position
        observed = wordle_engine.format_guess('abcde', 'aebcf')
        expected = f"{wc.GREEN}a{wc.YELLOW}e{wc.YELLOW}b{wc.YELLOW}c{wc.RED}f{wc.ENDC}"
        self.assertEqual(expected, observed)
    #     
